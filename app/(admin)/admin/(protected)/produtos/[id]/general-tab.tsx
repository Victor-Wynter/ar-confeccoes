...
export function GeneralTab({ product }: { product: Product }) {
  ...
  const {
    ...
    defaultValues: {
      ...
      basePriceBRL: product.basePrice / 100,
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="space-y-1.5">
          ...
          <Input id="edit-name" {...register("name")} onChange={onNameChange} />
      ...
      <div className="flex flex-wrap gap-6">
        <div className="flex items-center gap-3">
          <Switch id="edit-active" checked={active} onCheckedChange={(v) => setValue("active", v)} />
          <Label htmlFor="edit-active">Ativo</Label>
        </div>
        <div className="flex items-center gap-3">
          <Switch id="edit-featured" checked={featured} onCheckedChange={(v) => setValue("featured", v)} />
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">Ex: Material → Brim 100% algodão</p>
          </div>
              ...
              {fields.map((field, i) => (
                <div key={field.id} className="flex gap-2 items-start">
                  <div className="flex-1 space-y-1">
                    <Input placeholder="Label" {...register(`specifications.${i}.label`)} />
                    {errors.specifications?.[i]?.label && (
                      <p className="text-xs text-destructive">{errors.specifications[i].label?.message}</p>
                    )}
                  </div>
                  <div className="flex-1 space-y-1">
                    <Input placeholder="Valor" {...register(`specifications.${i}.value`)} />
                    {errors.specifications?.[i]?.value && (
                      <p className="text-xs text-destructive">{errors.specifications[i].value?.message}</p>
                    )}
                  </div>
                  <Button
                    type="button"
